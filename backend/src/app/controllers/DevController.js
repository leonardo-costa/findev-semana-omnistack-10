...
module.exports = {
  ...
  async store(req, res) {
    try {
      ...
      // check if dev has one of the techs and if he is near to 10km
      const sendSocketTo = findConnections({ latitude, longitude }, techsArray);

      sendMessage(sendSocketTo, 'new-dev', dev);
